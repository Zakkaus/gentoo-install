...
`--ram` と `--lowram` は実装済みで `cli.py` に接続されています。単体試験と plan 試験はメモリ環境を一度だけ起動するように設定する経路を対象としますが、その環境へ計算機を再起動した実行はありません。`dd` は実装済みです。単体試験と plan 試験は準備済みイメージをディスク全体へストリーム書き込みする処理を対象としますが、この方法で書き込まれたディスクはありません。
