...
ディスクへ導入する経路にはクラスタと単一マシンの記録があり、ext4、xfs、btrfs、f2fs、ZFS、LVM、mdraid、LUKS2 を、両方のファームウェアと両方の init システムで対象としています。稼働中のシステムをインプレース変換する経路には QEMU での記録が 4 件あり、BIOS が 2 件、`/home` を保持した UEFI が 1 件、ルートが btrfs で `/home` と `/var` を subvolume として保持した UEFI が 1 件です。この導入器自身が構築した計算機を変換する経路にはクラスタでの記録もあり、その経路はまだ信頼できません。再起動まで到達したクラスタ変換 5 件のうち 4 件は起動し、1 件はモジュールが見つからず GRUB の救助シェルで停止しましたが、変換自体の終了コードは `0` でした。
