...
| `ccca64926ccd3` | `vm-binpkg`, `vm-user-key`, `vm-sdboot`, `vm-zfs-encrypted`, `static-ip`, `vm-raidz`, `vm-zfs-mirror`, `ext2`, `ext3`, `zbm-unlock` — ten of eleven. `vm-user-key` is the first record of a key staged for an unprivileged account. The eleventh, `vm-home-key`, reached 51.5m and did not finish |
| `772fd462f5195` | `vm-cjk-kernel`, `vm-xfs`, `vm-btrfs`, `vm-mdraid`, `vm-zram`, `vm-lvm`, `openrc-sdboot`, `vm-luks`, `mbr-edit`, `ext4-bios`, `vm-unlock`, `vm-desktop` — twelve of twelve, the widest round with no failure of any kind. `vm-desktop` took 104.1m, which is what a Plasma group costs when every package is built from source |
| `5f17305f91d52` | `static-ip`, `vm-bios-luks`, `vm-zfs`, `vm-raidz`, `zfs-zbm`, `ext3`, `ext2`, `btrfs-luks`, `vm-greetd` — nine of twelve. `btrfs-luks` is its first pass: it had stopped at `install the plasma group` in the three rounds before, because the fixture paired `desktop = "plasma"` with the generic `.../desktop/systemd` profile. **The three that failed — `vm-zfs-encrypted`, `vm-zfs-mirror`, `zbm-unlock` — all stopped inside the same `emerge sys-apps/systemd`, and none of them is a fixture defect**: each console holds a second `starting serial terminal on interface serial0` with a fresh shell prompt under it, so the reconnecting `termproxy` hung up the guest's `ttyS0` and killed the foreground install. `zfs-zbm` ran the same emerge in the same round without a drop and passed, which is the counter-example. Fixed after this round; the fix has no record of its own yet |
| `1cb8337b4ff78` | `vm-luks`, `vm-xfs`, `vm-btrfs`, `vm-mdraid`, `openrc-sdboot` — five of twelve, and **the round that proves the pty fix and disproves the schedule that runs on it**. `vm-gnome`'s guest log holds **711 `INSTALL_BYTES` heartbeats** where the round before held none, so `script -q -e -c` puts the install back on a pty and the file grows through a long C++ link. **The three desktop fixtures then ran 482, 482 and 483 minutes and were ended by the run's eight-hour ceiling with the console still printing**, their last lines a `-std=c++20` compiler invocation, a Qt one, and `WebCore/CMakeFiles/WebCore.dir` — WebKit. They are not stalling; they are compiling. The same cause was read on a real machine the same night: a desktop profile moves a batch of USE flags, so almost nothing the official binhost built matches and Portage falls back to source. `vm-cjk-kernel` and `vm-lvm` failed on `POST .../termproxy did not answer`, which measures the API and not the install; both passed the round before. **What this does not establish:** whether a desktop fixture can pass on this cluster at all — the ceiling, the binhost USE alignment and the fixture set are one decision, recorded in `docs/tasks.md` row 543 |
...
| `902811d6bb7ca` | `vm-desktop` | the first run to read the input method configuration back. `verify app-i18n/rime-data installed luna_pinyin, pinyin_simp` passed as operation 68 of 70, against the target rather than against the plan; 370 packages from a binary host, 41 compiled, then the disk booted, mounted its layout and had no failed unit |
| `391bed77c5ceb` | `vm-shares`, on a 512 GiB disk | the first record of a partition sized as a share. The esp took its written `512MiB`, the root took `40%` of what that left as `209509MiB` -- inside its `20GiB` floor and its `300GiB` ceiling -- and `/home` took the rest. 58 operations, 30 packages from a binary host, 11 compiled, then the disk booted, mounted all three filesystems and had no failed unit |
| `391bed77c5ceb` | `vm-image` | the first record of an image this mode wrote being booted. 52 operations, 29 packages from a binary host, 11 compiled; the image came out of the guest over the SSH channel the run holds, became the disk the second pass attaches, and that disk booted, logged in on the console and passed every installed-state check. Every earlier row read the file with `losetup` and booted nothing |
