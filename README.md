...
Other implemented combinations remain unverified end to end. Current evidence does not cover greetd desktop sessions or ibus outside GNOME. It also does not cover the official Gentoo minimal ISO or Gig-OS live media, or binary-host failure fallback. The Alpine live medium is covered: at revision [`0827931289d0`](https://github.com/Zakkaus/gentoo-install/commit/0827931289d0), `alpine-standard-3.24.1` booted to a root shell on the serial console in 59 seconds, the installer wrote 56 operations with 57 packages from a binary host and 12 compiled, and the installed system booted, mounted its layout and reported no failed unit.
