...
The in-place conversion has one end-to-end record. At revision [`bcc090fab621`](https://github.com/Zakkaus/gentoo-install/commit/bcc090fab621), a Debian 12 genericcloud image running under QEMU on a single machine was converted in place and booted as Gentoo: `uname -r` reported `6.18.43-gentoo-dist-bin`, `emerge` was present, `dpkg` and `apt` were absent, the root filesystem was the same `/dev/vda1`, and the run's log was kept in `/var/log/gentoo-install`. That record covers one distribution, BIOS firmware and an ext4 root on a partition. The `vm-convert` cluster fixture has not yet passed, and conversion from any other distribution remains unverified.

Other implemented combinations remain unverified end to end. Current evidence does not cover greetd desktop sessions or ibus outside GNOME. It also does not cover the official Gentoo minimal ISO, Alpine or Gig-OS live media, or binary-host failure fallback.
