...
디스크에 설치하는 경로에는 클러스터와 단일 기계의 기록이 있으며 ext4, xfs, btrfs, f2fs, ZFS, LVM, mdraid, LUKS2를 두 펌웨어와 두 init 시스템에서 다룬다. 실행 중인 시스템을 인플레이스로 변환하는 경로에는 QEMU 기록이 세 건 있다. 두 건은 BIOS이고, 한 건은 자체 펌웨어 항목으로 부팅하며 `/home`을 보존한 UEFI 기록이다.

`--ram`과 `--lowram`은 각각 QEMU 기록이 하나씩 있다. Debian 12 기기가 한 번의 부팅을 설정하고, 기본 부팅 항목을 바꾸지 않은 채 재부팅하여 전달된 환경——`--ram`은 Gentoo CJK ISO, `--lowram`은 Alpine netboot 아카이브——으로 올라왔고, 전달받은 설정을 지니고 있었다. 그 환경 안에서 Gentoo를 설치한 기록은 없다. `dd`는 기록이 하나 있다. live 매체에서 준비된 이미지를 디스크 전체에 쓰고 raw와 gzip 두 형식 모두 바이트 단위로 읽어 냈다.

runner fixture 목록에는 고정 주소, ext2, ext3, 소스에서 빌드하는 커널이 있다. runner 수준의 시험은 binhost 폴백을 검증한다. 이 시험과 fixture는 엔드투엔드 기록이 아니다.
