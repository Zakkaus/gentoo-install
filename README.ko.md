...
디스크에 설치하는 경로에는 클러스터와 단일 기계의 기록이 있으며 ext4, xfs, btrfs, f2fs, ZFS, LVM, mdraid, LUKS2를 두 펌웨어와 두 init 시스템에서 다룬다. 실행 중인 시스템을 인플레이스로 변환하는 경로에는 QEMU 기록이 네 건 있다. 두 건은 BIOS이고, 한 건은 `/home`을 보존한 UEFI이며, 한 건은 루트가 btrfs이고 `/home`과 `/var`을 subvolume으로 유지한 UEFI 기록이다.
