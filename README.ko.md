...
`--ram`과 `--lowram`은 구현되어 `cli.py`에 연결되어 있다. 단위 시험과 plan 시험은 메모리 환경을 한 번 부팅하도록 설정하는 경로를 다루지만, 어느 기기도 그 환경으로 재부팅한 적이 없다. `dd`는 구현되어 있다. 단위 시험과 plan 시험은 준비된 이미지를 디스크 전체에 스트리밍하여 쓰는 동작을 다루지만, 이 방법으로 쓴 디스크는 없다.
