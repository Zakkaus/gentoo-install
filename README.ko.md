...
디스크에 설치하는 경로에는 클러스터와 단일 기계의 기록이 있으며 ext4, xfs, btrfs, f2fs, ZFS, LVM, mdraid, LUKS2를 두 펌웨어와 두 init 시스템에서 다룬다. 동작 중인 시스템을 인플레이스로 변환하는 경로의 기록은 둘이고, 모두 QEMU에서 나왔으며 모두 BIOS이다. 메모리 환경으로 부팅하도록 설정하는 경로는 구현되어 있고 단위 시험과 계획 시험이 이를 다룬다. 다만 어떤 실행도 기계를 그 환경으로 재부팅한 적이 없으므로 기록이 없다. 이미지를 기록하는 경로는 설계만 되어 있고 기록이 없다.
