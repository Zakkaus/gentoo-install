...
`--ram` reaches wifi and `--lowram` does not. The Gentoo CJK ISO carries NetworkManager and `linux-firmware`, so `nmcli device wifi connect <SSID> password <PASSWORD>` is the command that associates a link, and the first page says so in Traditional Chinese, Simplified Chinese and English. No run in `TESTED.md` has installed a machine over a wireless link, so what follows association is the design's claim and not a record. The Alpine netboot environment has no wireless driver and no supplicant, and its full module set is in a `modloop` that itself has to be fetched, so a machine whose only link is wifi cannot use `--lowram` at all.
